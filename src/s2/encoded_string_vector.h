...
class StringVectorEncoder {
 public:
  ...
  // REQUIRES: "encoder" uses the default constructor, so that its buffer
  //           can be enlarged as necessary by calling Ensure(int).
  static void Encode(s2_absl::Span<const string> v, Encoder* encoder);
...
class EncodedStringVector {
 public:
  ...
  s2_absl::string_view operator[](int i) const;
  ...
  // no longer needed.
  std::vector<s2_absl::string_view> Decode() const;
...
inline s2_absl::string_view EncodedStringVector::operator[](int i) const {
  uint64 start = (i == 0) ? 0 : offsets_[i - 1];
  uint64 limit = offsets_[i];
  return s2_absl::string_view(data_ + start, limit - start);
}
