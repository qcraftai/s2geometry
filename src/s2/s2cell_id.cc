...
S2CellId S2CellId::FromFaceIJ(int face, int i, int j) {
  ...
  // of the function.
  uint64 n = s2_absl::implicit_cast<uint64>(face) << (kPosBits - 1);
...
string S2CellId::ToString() const {
  if (!is_valid()) {
    return StrCat("Invalid: ", s2_absl::Hex(id(), s2_absl::kZeroPad16));
  }
...
S2CellId S2CellId::FromDebugString(s2_absl::string_view str) {
