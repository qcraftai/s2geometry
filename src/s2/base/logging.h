...
class S2LogMessage {
 ...
 private:
  bool enabled() const {
    ...
    return (static_cast<int>(severity_) >= S2_ABSL_MIN_LOG_LEVEL ||
...
class S2FatalLogMessage : public S2LogMessage {
 public:
  S2FatalLogMessage(const char* file, int line,
                    s2_absl::LogSeverity severity, std::ostream& stream)
      S2_ABSL_ATTRIBUTE_COLD
    : S2LogMessage(file, line, severity, stream) {}
  S2_ABSL_ATTRIBUTE_NORETURN ~S2FatalLogMessage() { abort(); }
};
...
#define S2_CHECK(condition) \
    S2_LOG_IF(FATAL, S2_ABSL_PREDICT_FALSE(!(condition))) \
