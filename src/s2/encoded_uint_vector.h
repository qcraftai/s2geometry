...
inline T GetUintWithLength(const char* ptr, int length) {
  ...
  if (length & sizeof(T)) {
    if (sizeof(T) == 8) return S2_ABSL_INTERNAL_UNALIGNED_LOAD64(ptr);
    if (sizeof(T) == 4) return S2_ABSL_INTERNAL_UNALIGNED_LOAD32(ptr);
    if (sizeof(T) == 2) return S2_ABSL_INTERNAL_UNALIGNED_LOAD16(ptr);
  ...
  if (sizeof(T) > 4 && (length & 4)) {
    x = S2_ABSL_INTERNAL_UNALIGNED_LOAD32(ptr -= sizeof(uint32));
  }
  if (sizeof(T) > 2 && (length & 2)) {
    x = (x << 16) + S2_ABSL_INTERNAL_UNALIGNED_LOAD16(ptr -= sizeof(uint16));
  }
