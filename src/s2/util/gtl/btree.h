...
struct is_comparator_transparent<Compare,
                                 s2_absl::void_t<typename Compare::is_transparent>>
    : std::true_type {};
...
template <>
struct key_compare_to_adapter<std::less<s2_absl::string_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::less<s2_absl::string_view> &) {}
    int operator()(const s2_absl::string_view a, const s2_absl::string_view b) const {
      return a.compare(b);
    }
  };
};
...
template <>
struct key_compare_to_adapter<std::greater<s2_absl::string_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::greater<s2_absl::string_view> &) {}
    int operator()(const s2_absl::string_view a, const s2_absl::string_view b) const {
      return b.compare(a);
    }
  };
...
struct has_linear_node_search_preference<
    T, s2_absl::void_t<typename T::goog_btree_prefer_linear_node_search>>
    : std::true_type {};
template <typename T>
...
struct common_params {
  ...
  using key_compare =
      s2_absl::conditional_t<btree_is_key_compare_to<Compare>::value, Compare,
  ...
  using node_count_type =
      s2_absl::conditional_t<(kNodeValueSpace / ValueSize >
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize,
  ...
  using slot_type = s2_absl::container_internal::slot_type<Key, mapped_type>;
  ...
  struct value_compare : private key_compare {
    ...
    s2_absl::conditional_t<btree_is_key_compare_to<key_compare>::value, int, bool>
...
struct set_params
  ...
  // s2_absl::container_internal::slot_type interface.
  struct slot_type {
    value_type value;

    template <class... Args>
    static void construct(Alloc *alloc, slot_type *slot, Args &&... args) {
      s2_absl::allocator_traits<Alloc>::construct(*alloc, &slot->value,
    ...
    static void destroy(Alloc *alloc, slot_type *slot) {
      s2_absl::allocator_traits<Alloc>::destroy(*alloc, &slot->value);
    }
...
class btree_node {
 ...
 public:
  ...
  using use_linear_search = s2_absl::conditional_t<
 ...
 public:
  ...
  void clear_child(int i) {
    s2_absl::container_internal::SanitizerPoisonObject(&mutable_child(i));
  }
  void set_child(int i, btree_node *c) {
    s2_absl::container_internal::SanitizerUnpoisonObject(&mutable_child(i));
    mutable_child(i) = c;
    c->set_position(i);
  }
  ...
  static btree_node *init_leaf(btree_node *n, btree_node *parent,
    ...
    s2_absl::container_internal::SanitizerPoisonMemoryRegion(
 ...
 private:
  ...
  void value_init(const size_type i, allocator_type *alloc, Args &&... args) {
    s2_absl::container_internal::SanitizerUnpoisonObject(slot(i));
    slot_type::construct(alloc, slot(i), std::forward<Args>(args)...);
  }
  void value_destroy(const size_type i, allocator_type *alloc) {
    slot_type::destroy(alloc, slot(i));
    s2_absl::container_internal::SanitizerPoisonObject(slot(i));
  }
  ...
  void uninitialized_move_n(const size_type n, const size_type i,
                            ...
                            allocator_type *alloc) {
    s2_absl::container_internal::SanitizerUnpoisonMemoryRegion(
...
template <typename T>
using is_trivially_copyable = s2_absl::conjunction<
    s2_absl::is_trivially_copy_constructible<T>,
    s2_absl::disjunction<s2_absl::is_trivially_copy_assignable<T>,
                      s2_absl::negation<std::is_copy_assignable<T>>>,
    s2_absl::is_trivially_destructible<T>>;
...
class btree {
  ...
  template <typename K>
  using const_lookup_key_reference = s2_absl::conditional_t<
 ...
 public:
  ...
  btree(btree &&x) noexcept
      : root_(std::move(x.root_)),
        rightmost_(s2_absl::exchange(x.rightmost_, nullptr)),
        size_(s2_absl::exchange(x.size_, 0)) {
 ...
 private:
  ...
  node_type *allocate(const size_type size) {
    return reinterpret_cast<node_type *>(
        s2_absl::container_internal::Allocate<node_type::Alignment()>(
  ...
  void deallocate(const size_type size, node_type *node) {
    s2_absl::container_internal::Deallocate<node_type::Alignment()>(
 ...
 private:
  ...
  s2_absl::container_internal::CompressedTuple<key_compare, allocator_type,
                                            node_type *>
      root_;
  ...
  static_assert(std::is_same<s2_absl::result_of_t<key_compare(key_type, key_type)>,
                             s2_absl::conditional_t<is_key_compare_to::value, int,
                                                 bool>>::value,
