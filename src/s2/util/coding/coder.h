...
inline bool Encoder::PutVarint64FromDecoderLessCommonSizes(Decoder* dec) {
  ...
  if (S2_ABSLPREDICT_TRUE(dec_ptr <= dec_limit - kVarintMax64 &&
...
S2_ABSLATTRIBUTE_ALWAYS_INLINE inline bool Encoder::put_varint64_from_decoder(
    Decoder* dec) {
  unsigned char* enc_ptr = buf_;
  const unsigned char* dec_ptr = dec->buf_;

  // Common cases to handle varint64 with one to two bytes.
  if (S2_ABSLPREDICT_TRUE(dec_ptr < dec->limit_ && dec_ptr[0] < 128)) {
    if (S2_ABSLPREDICT_FALSE(enc_ptr >= limit_)) {
      return false;
    }
    *enc_ptr = *dec_ptr;
    dec->buf_++;
  ...
  if (dec_ptr < dec->limit_ - 1 && dec_ptr[1] < 128) {
    if (S2_ABSLPREDICT_FALSE(enc_ptr >= limit_ - 1)) {
      return false;
    }
