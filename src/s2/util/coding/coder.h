...
inline bool Encoder::PutVarint64FromDecoderLessCommonSizes(Decoder* dec) {
  ...
  if (S2_ABSL_PREDICT_TRUE(dec_ptr <= dec_limit - kVarintMax64 &&
...
S2_ABSL_ATTRIBUTE_ALWAYS_INLINE inline bool Encoder::put_varint64_from_decoder(
    Decoder* dec) {
  unsigned char* enc_ptr = buf_;
  const unsigned char* dec_ptr = dec->buf_;

  // Common cases to handle varint64 with one to two bytes.
  if (S2_ABSL_PREDICT_TRUE(dec_ptr < dec->limit_ && dec_ptr[0] < 128)) {
    if (S2_ABSL_PREDICT_FALSE(enc_ptr >= limit_)) {
      return false;
    }
    *enc_ptr = *dec_ptr;
    dec->buf_++;
  ...
  if (dec_ptr < dec->limit_ - 1 && dec_ptr[1] < 128) {
    if (S2_ABSL_PREDICT_FALSE(enc_ptr >= limit_ - 1)) {
      return false;
    }
...
inline void DecoderExtensions::FillArray(Decoder* array, int num_decoders) {
  ...
  static_assert(s2_absl::is_trivially_copy_constructible<Decoder>::value,
                "Decoder must be trivially copy-constructible");
  static_assert(s2_absl::is_trivially_copy_assignable<Decoder>::value,
                "Decoder must be trivially copy-assignable");
  static_assert(s2_absl::is_trivially_destructible<Decoder>::value,
...
inline void Encoder::putfloat(float f) {
  put32(s2_absl::bit_cast<uint32>(f));
}

inline void Encoder::putdouble(double d) {
  put64(s2_absl::bit_cast<uint64>(d));
}
...
inline float Decoder::getfloat() {
  return s2_absl::bit_cast<float>(get32());
}

inline double Decoder::getdouble() {
  return s2_absl::bit_cast<double>(get64());
}
