...
class InlinedVector {
 ...
 public:
  ...
  InlinedVector& operator=(const InlinedVector& other) {
    if (S2_ABSLPREDICT_FALSE(this == &other)) return *this;
  ...
  reference emplace_back(Args&&... args) {
    size_type s = size();
    assert(s <= capacity());
    if (S2_ABSLPREDICT_FALSE(s == capacity())) {
  ...
  void shrink_to_fit() {
    const auto s = size();
    if (S2_ABSLPREDICT_FALSE(!allocated() || s == capacity())) return;
...
auto InlinedVector<T, N, A>::emplace(const_iterator position, Args&&... args)
  ...
  assert(position >= begin());
  assert(position <= end());
  if (S2_ABSLPREDICT_FALSE(position == end())) {
...
void InlinedVector<T, N, A>::swap(InlinedVector& other) {
  ...
  if (S2_ABSLPREDICT_FALSE(this == &other)) return;
...
auto InlinedVector<T, N, A>::InsertWithCount(const_iterator position,
  ...
  if (S2_ABSLPREDICT_FALSE(n == 0)) return const_cast<iterator>(position);
...
auto InlinedVector<T, N, A>::InsertWithRange(const_iterator position,
    ...
    -> iterator {
  assert(position >= begin() && position <= end());
  if (S2_ABSLPREDICT_FALSE(first == last)) return const_cast<iterator>(position);
