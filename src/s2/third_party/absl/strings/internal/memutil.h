...
// (they use s2_absl::ascii_tolower instead of tolower).
//
...
#include "s2/third_party/absl/strings/ascii.h"  // for s2_absl::ascii_tolower
...
const char* int_memmatch(const char* haystack, size_t haylen,
  ...
  for (; haystack < hayend; ++haystack) {
    char hay = case_sensitive
                   ? *haystack
                   : s2_absl::ascii_tolower(static_cast<unsigned char>(*haystack));
    char nee = case_sensitive
                   ? *needle
                   : s2_absl::ascii_tolower(static_cast<unsigned char>(*needle));
    if (hay == nee) {
      if (++needle == needleend) {
